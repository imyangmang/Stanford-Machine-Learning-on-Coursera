...
z3 = a2 * Theta2';
a3 = sigmoid(z3);
Y = zeros(m,num_labels);%reshape lable y 5000 by 10
%有没有更简便的实现方法？
for i =1:m
    Y(i,y(i)) = 1;
end
...
regu = sum(sum(Theta1(:,2:end).*Theta1(:,2:end))) + sum(sum(Theta2(:,2:end).*Theta2(:,2:end)));
J = 1/m * J + lambda / (2*m) * regu;
...
Theta1_grad = 1/m * (Theta1_grad + delta2(:,2:end)' * a1);
